...
stats 'points_nn_back.csv' using 4 nooutput
cost_min = STATS_min
cost_max = STATS_max
...
plot \
    'points_nn_back.csv' using 2:3:($4 - cost_min)/(cost_max - cost_min):(scale($4)) \
        with points pt 7 ps variable lc palette title 'All Nodes', \
    'solution_nn_back.csv' using 1:2 with lines lw 2 lc rgb "blue" title 'Path', \
    'points_nn_back.csv' using ($5==1 ? $2 : 1/0):($5==1 ? $3 : 1/0):($4 - cost_min)/(cost_max - cost_min):(scale($4)) \
